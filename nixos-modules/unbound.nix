  ...
  services.unbound = {
    ...
    settings = {
      server = {
        ...
        # Local DNS entries.
        local-data = let
          inherit (config.role-configuration) domain;

          # Create an error message for duplicate IP addresses.
          grouped = builtins.groupBy (device: device.ip-address) config.role-configuration.devices;
          ...
          devices =
            ...
            then config.role-configuration.devices
        ...
        in
          ...
          ++ (map (device: ''"${device.name}.${domain}. IN A ${device.ip-address}"'') devices)
